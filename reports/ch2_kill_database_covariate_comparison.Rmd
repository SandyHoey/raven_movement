...
data %>% 
  ...
  summarize(mean_kill_density = mean(avg_terr_kill_density),
            ...
            n_active_kills = sum(active_kill),
            rf_n_active_kills = sum(rf_active_kill))
...
#data for part 1 of conditional model
ws_model_data <- read_csv(here("data", "clean", "commute_data.csv")) %>% 
  ...
  dplyr::select(terr_bin, raven_id, active_kill, rf_active_kill, bms_window_1, hunt_season, take_high_low, avg_terr_kill_density, rf_avg_terr_kill_density, dist2nentrance,
...
mod_wp <- glmer(terr_bin ~ (1|raven_id) + active_kill * scale(bms_window_1) + scale(avg_terr_kill_density) + 
...
```{r RF covariate}
mod_rf <- glmer(terr_bin ~ (1|raven_id) + rf_active_kill * scale(bms_window_1) + scale(rf_avg_terr_kill_density) + 
                         scale(dist2nentrance) + study_period * scale(temp_max) + scale(snow_depth) + scale(prop_group_left_terr),
                       data = ws_model_data,
                       family = "binomial",
                       nAGQ = 40,
...
summary(mod_rf)
```
The models have basically the same results. Not sure what to think. 
