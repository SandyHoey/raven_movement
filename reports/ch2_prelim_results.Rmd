...
- `final_take_bms`: the estimated available biomass from human hunting adjusted for relative weights of various species (the multiplier for each is 0.35\*deer, 1\*elk, 2.15\*bison). The biomass from the previous day is rolled over at 0.25x rate
...
boot_param_CI(nsim = 50, model = mod_terr_bms, data = ws_model_data)[[3]]
```
```{r (1) bootstrap (season), echo=FALSE, warning=FALSE}
#bootstrapping parameter values from model simulations
boot_param_CI(nsim = 50, model = mod_terr_hseason, data = ws_model_data)[[3]]
```
...
#bootstrapping parameter values from model simulations
boot_param_CI(nsim = 50, model = mod_hunt_hseason, data = hunt_model_data)[[3]]
```
## Figures
...
source(here("scripts", "wolf_kill_visits.R"))
